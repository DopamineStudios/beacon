...
class BeaconFrameworkBotMixin:
    ...
    def __init__(self, cogs_path: str = "cogs", default_diagnostics: bool = True, default_help_command: commands.HelpCommand | None = None,
        ...
        Args:
            ...
            on_shard_ready_callback: A synchronous or asynchronous function to execute for each shard when using AutoShardedBot. Use functools.partial if it needs any custom arguments other than shard_id. Useful for purposes such as setting a custom Bot status for each shard like "Running on shard 67 of 69", etc.
            on_beacon_ready_callback: A synchronous or asynchronous function to execute after bot is ready. Use functools.partial if it needs custom arguments. Unlike on_shard_ready_callback, this will always be executed (if a callback is provided) regardless of if you use sharding or not. This is meant to allow developers to execute custom code after bot is ready because the traditional way of using setup_hook or on_ready won't work without breaking Beacon. Please note that this can be called multiple times.
